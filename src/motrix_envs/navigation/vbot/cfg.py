...
class VBotSection01EnvCfg(VBotStairsEnvCfg):
    ...
    @dataclass
    class Commands:
        # 路径点导航：沿 Y 轴前进方向依次经过 7 个绝对世界坐标 (X, Y)，
        # 进入每个点 waypoint_reach_threshold 范围内即视为到达并切换到下一个点，
        # 不需要在每个点停留；到达最后一个点才算真正完成本赛段。
        waypoint_targets = (
            (0.0, -0.60),
            (0.0, 1.20),
            (0.0, 2.25),
        ...
        waypoint_reach_threshold = 0.45   # 到达判定半径 [m]
        waypoint_lin_kp = 0.8             # 位置误差 -> 期望线速度指令的比例系数
        waypoint_ang_kp = 1.0             # 朝向误差 -> 期望角速度指令的比例系数
        waypoint_cmd_smooth_tau = 0.25    # 指令一阶低通滤波时间常数 [s]
        # 旧的单目标随机采样范围，仅保留形状供其他代码路径兼容引用，
        # 路径点导航启用后不再用它生成目标。
        pose_command_range = [0.0, 10.2, 0.0, 0.0, 10.2, 0.0]
    @dataclass
    class ControlConfig:
    ...
    @dataclass
    class RewardConfig:
        # 三大类奖励：导航跟踪 / 稳定性与控制正则 / 步态与分段塑形。
        # 分段塑形项只在特定 Y 区间生效（见 _compute_reward 里各自的 zone mask），
        # 权重和分段边界都是针对 section01 赛道的五段地形（起步平台/崎岖区/落差坎/
        # 15°上坡/2026平台）专门调出来的，不是通用 locomotion 默认值。
        scales: dict[str, float] = field(
            default_factory=lambda: {
                # ===== 导航跟踪 =====
                ...
                "drop_leg_catchup": 4.0,
                "drop_pitch": -2.0,
                "slope_leg_drive": 3.0,
                "slope_front_drive": 3.0,
                "slope_hip": -1.0,
                # slope_pitch 不单独设权重：上坡俯仰误差已经并入 orientation 统一处理。
            }
        )
    init_state: InitState = field(default_factory=InitState)
    commands: Commands = field(default_factory=Commands)
